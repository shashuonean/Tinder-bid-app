...
<head>
    ...
    <!-- 
        *** FIX: Load Firebase Dependencies using standard script tags (not type="module") 
        This prevents conflicts with Babel/JSX on GitHub Pages.
    -->
    ...
    <!-- 
        *** FIX: Initialize Firebase Globally AFTER dependencies are loaded 
    -->
    <script>
        ...
        const initializeFirebase = () => {
            ...
            if (Object.keys(firebaseConfig).length > 0 && typeof firebase !== 'undefined') {
                try {
                    ...
                    window.dbInstance = firebase.firestore.getFirestore(app);
                    window.authInstance = firebase.auth.getAuth(app);
                    window.appId = appId;
                    window.initialAuthToken = typeof __initial_auth_token !== 'undefined' ? __initial_auth_token : null;
                    window.isFirebaseReady = true;
                    // Trigger a re-render check if React is already loaded
                    if (typeof window.triggerFirebaseReady === 'function') {
                 ...
                 console.warn("Firebase config or dependencies missing. App running without database.");
            }
        };

        // Initialize immediately after the script block loads
...
<body class="bg-gray-900">
    ...
    <script type="text/babel">
        ...
        const { getFirestore, doc, setDoc, getDoc, collection, query, onSnapshot, where, addDoc, updateDoc, writeBatch } = firebase.firestore || {};
        ...
        const ReviewAwardModal = ({ isOpen, onClose, tender, bids, onAwardConfirm, showAlert }) => {
            ...
            const handleAward = async (bid) => {
                // IMPORTANT: Using custom modal UI/prompt simulation instead of confirm()
        ...
        const AppContent = () => {
            ...
            const handlePaymentConfirm = async (tender, bid) => {
                ...
                try {
                    const batch = writeBatch(db);
                    const tenderDocRef = doc(db, 'artifacts', appId, 'public', 'data', 'tenders', tender.id);
                    batch.update(tenderDocRef, { status: 'Paid', paymentDate: new Date() });
        ...
        const App = () => {
             ...
             useEffect(() => {
                 // Set a global function that the script can call once firebase is initialized
                 window.triggerFirebaseReady = () => setIsLoaded(true);
